...
-- alongside their tables in 003/004 (they are on GENERATED columns and cannot be added
-- independently of the column). This migration holds search *tuning* that is separable from
-- shape: wider column statistics for the planner, and a smaller GIN pending-list so interactive
-- search stays current between autovacuum runs. Transcribed verbatim from Database.md's "## DDL"
-- block.
