...
-- migration runner). Extensions, the okf_english text-search configuration, every enum type, the
-- shared set_updated_at() trigger function, and the schema_migrations ledger table itself.
-- Transcribed verbatim from Database.md's "## DDL" block — that document is the only authority on
-- shape; if this file and Database.md ever disagree, Database.md changes first and this file
-- follows. Every CREATE here is idempotent (IF NOT EXISTS, or a DO $$ … $$ guard for the enums,
-- which have no CREATE TYPE IF NOT EXISTS), so re-running this file against a partially-migrated
-- database is safe.
